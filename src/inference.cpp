...
int Inference::init_inf() {
    ...
    for (int m = 0; m < pnewData->numDocs; m++) {
        ...
        for (int t = 0; t < docLength; t++) {
            ...
            // sample sentiment label 0-neural
            if ( pnewData->pdocs[m]->priorSentiLabels[t] != 0 ) {
...
int Inference::inference() {
    ...
    for (int it = 1; it <= niters; it++) {
        ...
        if (savestep > 0 && it % savestep == 0) {
            ...
            compute_newtheta();
            compute_newphi();
            save_model(putils->generate_infer_name(it));
        }
    }
    ...
    compute_newtheta();
    compute_newphi();
    save_model(putils->generate_infer_name(-1));
...
int Inference::read_newData(string filename) {
    ...
    if (sentiment_lexicon_dir != "") {
        if (pnewData->read_senti_lexicon((sentiment_lexicon_dir).c_str())) {
            printf("Error! Cannot read sentiFile %s!\n", sentiment_lexicon_dir.c_str());
            delete pnewData;
            return 1;
        }
        else {
            this->word_positive = pnewData->positive_lexicon;
            this->word_negative = pnewData->negative_lexicon;
            for (int i = 0; i < word_positive.size(); i++ ) {
                word2prior.insert(pair<string, int>(word_positive[i],1));
            }
            for (int i = 0; i < word_negative.size(); i++) {
                word2prior.insert(pair<string, int>(word_negative[i],2));
            }
        }
    }
...
//                pnewData->newWords.push_back(strtok.token(k).c_str());
                ...
                _doc.push_back(_id);
                if (word2prior.find(it->first) != word2prior.end()) {
                    priorSentiLabels.push_back(word2prior[it->first]);
                }else{
                    priorSentiLabels.push_back(0);
                }
            }
        }
...
int Inference::read_model_setting(string filename) {
    ...
    if (numSentiLabs == 0 || numTopics == 0 || numDocs == 0 || vocabSize == 0) {
        printf("Throw exception in reading model parameter settings : %s \n", filename.c_str());
        return 1;
    }
    else {
        printf("data_dir = %s\n", data_dir.c_str());
        printf("datasetFile = %s\n", datasetFile.c_str());
        printf("result_dir = %s\n", result_dir.c_str());
        ...
        printf("numTopics = %d\n", numTopics);
        printf("numSentiLabs = %s\n", data_dir.c_str());
        printf("niters = %s\n", data_dir.c_str());
        printf("savestep = %s\n", data_dir.c_str());
        printf("twords = %s\n", data_dir.c_str());
        printf("updateParaStep = %s\n", data_dir.c_str());
    }
...
int Inference::prior2beta() {
    ...
    for (int l = 0; l < numSentiLabs; l++) {
        ...
        for (int z = 0; z < numTopics; z++) {
            ...
            for (int r = 0; r < pnewData->vocabSize; r++) {
                beta_lzw[l][z][r] = lambda_lw[l][_id2id[r]] * 0.01;
...
int Inference::save_model(string model_name) {
    if (save_model_newtassign(result_dir + model_name + tassign_suffix)) return 1;
    if (save_model_newtwords(result_dir + model_name + twords_suffix)) return 1;
    if (save_model_newpi_dl(result_dir + model_name + pi_suffix)) return 1;
    if (save_model_newtheta_dlz(result_dir + model_name + theta_suffix)) return 1;
    if (save_model_newphi_lzw(result_dir + model_name + phi_suffix)) return 1;
    if (save_model_newothers(result_dir + model_name + others_suffix)) return 1;
    return 0;
}
